...
import org.SirTobiSwobi.c3.ntfc.db.Assignment;
...
import org.SirTobiSwobi.c3.ntfc.db.Evaluation;
...
import org.SirTobiSwobi.c3.ntfc.db.TrainingSession;
import org.SirTobiSwobi.c3.ntfc.db.WordEmbedding;

public class DaiFold extends Fold {

	public DaiFold(ReferenceHub refHub, long[] trainingIds, long[] evaluationIds, int foldId, long modelId,
				...
				for(int j=0; j<clusters.size(); j++){
					for(int k=0;k<categories.length; k++){
						...
						if(probability>assignmentThreshold){
							if(evalCznMan.containsCategorizationOf(evaluationIds[i], cat.getId())){
								Categorization czn = evalCznMan.getCategorizationWithDocAndCat(evaluationIds[i], cat.getId());
								if(czn.getProbability()<probability){
									czn.setProbability(probability);
								}
								evalCznMan.setCategorization(czn);
								appendString+="Updated document "+evaluationIds[i]+" to category "+cat.getId()+" with probability "+probability+". <br />";
							}else{
								evalCznMan.addCategorizationWithoutId(evaluationIds[i], cat.getId(), probability, "");
								appendString+="Assigned document "+evaluationIds[i]+" to category "+cat.getId()+" with probability "+probability+". <br />";
							}
